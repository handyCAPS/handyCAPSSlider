/*! handyCAPSSLider - v0.1.0 - 2014-05-04
...
function HandyCAPSSlider(cont)  {
	...
	this.getTransformString = function(perc, deg) {
		var degs = deg || parseInt(perc, 10) * 360,
	...
	this.moveSlides = function () {
		...
		if (this.i >= numItems - 1) {
			...
			for (var i = 0; i < numItems; i++) {
				slide[i].style.transform = this.getTransformString(0,0);
				slide[i].style.webkitTransform = this.getTransformString(0,0);
			}
	...
	this.init = function(paras) {
		...
		this.pauseHover = params.pauseOnHover === undefined || true ? true : false;
